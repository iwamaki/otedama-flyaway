...
★ルール:
├─ オフセット(x,y)は常に 0,0 を使う
├─ verticesに実座標を直接記載
├─ 矩形の場合: 左上→右上→右下→左下 の順
...
★設計ルール:
├─ ジャンプ先の足場は必ず視界内に配置する
├─ 視界外へのジャンプは禁止（プレイヤーが見えない）
├─ 横方向10マス以上離れた足場は見えない
├─ 上方向20マス以上離れた足場は見えない
└─ 下方向15マス以上離れた足場は見えない

例: 現在位置から y=-18 の足場へジャンプ
    → 上方向20マス以内なのでOK（視界内）

例: 現在位置から x=15 の足場へジャンプ
    → 横方向10マス超えなので視界外（設計NG）
    → 途中に足場を設置するか、斜め移動で近づいてから
```
...
★座標の書き方（JSON直結形式）:
├─ x, y は常に 0, 0 にする（オフセットなし）
├─ vertices に実座標を直接記述
├─ 矩形: 左上→右上→右下→左下 の順
└─ ドキュメントの「x=左端〜右端, y=上端〜下端」がそのまま使える

terrainType一覧:
├─ grass    : 草地（friction: 0.5, restitution: 0.2）
├─ dirt     : 土（friction: 0.5, restitution: 0.2）
├─ rock     : 岩（friction: 0.6, restitution: 0.3）
...
- [ ] 遷移前後の足場x座標が重複していない（斜めジャンプで乗れる配置）
- [ ] 壁接続の足場で端落ちを救済
